...
const apiServerStatus = () => {
  return {httpStatus: 200, status: '200', resData: {message: 'Kaia Mart Vendor Customers - Items API server is running'}};
}
...
const createFail = (resData) => {
  return {httpStatus: 500, status: '196', resData};
}

const updateFail = (resData) => {
  return {httpStatus: 500, status: '195', resData};
}
module.exports = {
  apiServerStatus, unKnownErr, noData, 
  tokenRequired, mandatory, tokenInvalid,
  tokenExpired, invalidAccess,  responseData,
  createFail, updateFail
}
