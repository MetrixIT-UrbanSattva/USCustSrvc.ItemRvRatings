...
// --- Begin: Customers Item Review Ratings Schema --- //
const schema = new Schema({
  ...
  hoda: {type: String, required: true}, // Sale to Customer Status: Created, Active, Inactive
...
schema.index({vndrOrg: 1, citd: 1, uId: 1, so: 1, soItem: 1}, {unique: true});
schema.index({delFlag: -1, voiCode: 1, soiCode: 1, hoda: 1, isReview: 1});
schema.index({hCount: 1, lCount: 1, rating: 1, cDtNum: -1, uDtNum: -1});

module.exports = mongoose.model(config.collCustsItemRvRtngs, schema);
// --- End: Customers Item Review Ratings Schema --- //
