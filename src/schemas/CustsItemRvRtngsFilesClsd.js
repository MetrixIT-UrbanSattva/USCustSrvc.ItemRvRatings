...
// --- Begin: Customers Item Review Ratings Files Closed Schema --- //
const schema = new Schema({
  _id: {type: String, default: uuidv4()},
  cirrc: {type: String, required: true, ref: config.collCustsItemRvRtngsClsd},
...
}, {collection: config.collCustsItemRvRtngsFilesClsd});

schema.index({vndrOrg: 1, cirrc: 1, citdc: 1, path: 1}, {unique: true});
schema.index({delFlag: 1, voiCode: 1, soiCode: 1, hoda: 1});

module.exports = mongoose.model(config.collCustsItemRvRtngsFilesClsd, schema);
// --- End: Customers Item Review Ratings Files Closed Schema --- //
